...
router.post('/playground/generate', upload.single('face_image'), async (req, res) => {
    ...
    try {
        ...
        helpers.delay(10000).then(async () => { // Remaker's servers seem to fail consistently when prompted right away
            ...
            if(!remakerGetResponse.data.result.output_image_url) return res.status(500).send('Remaker service failed to finish generating face swap.');
            ...
            if (!saveNewGenerationItem(newGenerationItem)) return res.status(500).send('Failed to save the new image generation item to the server database.');
...
async function combineClothingProperties(clothingCategories, clientCategoriesSelections) {
    ...
    for (let i = 0; i < clientCategoriesSelections.length; i++) {
        const categoryDocument = clothingCategories.find(obj => obj['_id'] === clientCategoriesSelections[i]['category_id']); // need category name first to know which collection to target for clothing item name
